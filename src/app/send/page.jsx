...
import { useForm } from "react-hook-form";
import Link from "next/link";
import { useRouter } from "next/navigation"; // Fixed typo from 'next/navigation' to 'next/router'
import { ArrowLeft, Scan, ArrowsLeftRight } from "@phosphor-icons/react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    <section className="home-background h-screen flex flex-col p-5 xl:px-[200px]">
      <div className="flex justify-between">
        <span className="flex flex-col items-center w-full">
          <span className="flex items-center justify-between w-full mb-3">
            <Link href="/home">
              <span><ArrowLeft size={24} color="#ffffff" /></span>
            </Link>
            <h3 className="text-[#A4A4A4] text-lg ">Send Crypto</h3>
            <Scan size={24} color="#ffffff" />
          </span>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-10">
        ...
        {amount && (
          <div className="text-white mt-4">
            <p>Transaction Fee: {transactionFee.toFixed(2)} USDC</p>
          </div>
        )}
        <button className="bg-white font-bold text-lg p-3 rounded-xl w-full mt-5">
          Send
      ...
      <Dialog open={openSuccess} onOpenChange={setOpenSuccess}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="mb-[20px]">
              {finalAmount} {currency.toUpperCase()} Transferred Successfully
            </DialogTitle>
            <Player
              keepLastFrame
              autoplay
